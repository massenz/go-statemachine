...
func (s *SqsPublisher) Publish(topic string) {
    ...
    for msg := range s.errors {
        ...
        msgResult, err := s.client.SendMessage(&sqs.SendMessageInput{
            ...
            // TODO: this whole thing will be replaced as we move away from EventErrorMessages
            // and to native Protobuf error response.
            MessageBody: aws.String(proto.MarshalTextString(msg.Message)),
            QueueUrl:    &queueUrl,
        })
        if err != nil {
